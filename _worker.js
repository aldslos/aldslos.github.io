...
<meta property="og:site_name" content="es.tseb.eu">
	...
	async fetch(request, env, ctx) {
		try {
			...
			if (!self.verificationList) {
				...
				const res = await fetch("https://es.tseb.eu/url-sitemap.txt");
			...
			if (pathname === "/googleedccd1e4777880fc.html") {
				const fileRes = await fetch("https://ndende.eu/googleedccd1e4777880fc.html");
			...
			if (pathname === "/" || pathname.startsWith("/page/")) {
				...
				if (!urlParams.has("lang")) urlParams.set("lang", "es");
				if (!urlParams.has("data")) urlParams.set("data", "010");
				if (!urlParams.has("random")) urlParams.set("random", "1");
			...
			const staticExtensions = ['.ico', '.txt', '.txt.gz', '.xml', '.xml.gz', '.aldslos'];
